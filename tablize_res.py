...
def get_res_per_file(file):
    def label_text_length(inputs):
        ...
        if length_dict[0.75] <= 10:
            ...
            if length_dict[0.25] > 10 and length_dict[0.75] <= 50:
                return ["medium"]
            elif length_dict[0.25] < 10:
                return ["small"]
    ...
    for idx, results in pairs:
        ...
        index["label size"] = (lambda x: ["single" if "_".join(x[0]['task']['data_config']['huggingface_dataset_name']) in vars.multilabel_ds else "multilabel"], [])
