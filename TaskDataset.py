...
from vars import split_ratio, split_strategies, imb_ratio
...
def set_imbalance_by_cls(no_by_cls:dict, tolerance, threshold, cls_ratio_to_imb=0.75, sample_ratio_to_imb=0.5):
    ...
    imb_no = floor(len(no_by_cls) * cls_ratio_to_imb)
    ...
    bcls_count = [(cls, floor(count * sample_ratio_to_imb)) for cls, count in bcls_count]
...
def set_imb_count_dict(count_dict:dict, tolerance: float, threshold: float,
                       cls_ratio_to_imb, sample_ratio_to_imb, split_strategy: str = None):

    test_no_by_label = {lb: floor(count_dict[lb] * split_ratio['test']) for lb in count_dict}
    if not split_strategy:
        train_no_by_label = {lb: floor((count_dict[lb] - test_no_by_label[lb]) * split_ratio['train'])
                             for lb in count_dict}
    if split_strategy is "undersample":
        train_no_by_label = _get_split_amount(count_dict, split_strategies.index(split_strategy),
                                              split_ratio['train'], tolerance)
    ...
    assert is_imbalanced_ds(train_no_by_label, tolerance, threshold) or \
           is_imbalanced_ds(train_no_by_label, tolerance-0.1, threshold-0.1)
...
def split_tds(filename:str, split_strategy=None):
    ...
    test_no_by_label = {lb: floor(count_dict[lb] * split_ratio['train']) for lb in count_dict}
...
def _get_split_amount(count_dict:dict, strategy:int, ratio:float, tolerance:float) -> dict:
    ...
    if strategy == 0:
        ...
        out = {cls: floor(bavg*ratio) if is_imbalanced_cls(cls, count_dict, tolerance) and count_dict[cls] > bavg
                                        ...
                                        else floor(count_dict[cls]*ratio) for cls in count_dict}
